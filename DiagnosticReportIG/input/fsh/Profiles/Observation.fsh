...
* meta
  * tag ^definition = "Relevant searchable codes are located in various elements (code, value) depending on the type of investigation. They are all collected in tag for easier search. In hierachical sets of observations the codes may also be at various depth in the hierachy. All codes are in such cases collected in this element in the top-level observation. Hence a hit on a code in tag will always return a top-level observation, from which any hierarchy only needs to be nested inwards."
